...
Example:
    ...
    Output:
       ...
       //System.out.println("Second Max: " + nums[nums.length - 2]); this doesn't work because 8 is duplicate, so it gives wrong second max
        ...
        for (int i = nums.length - 1; i >= 0; i--) { // read the number from the end, We sorted so I will keep running until I find a new number that is not the same as the max number
            if (nums[i] != max) {
